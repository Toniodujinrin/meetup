...
conversationSchema.post<ConversationInterface>(
  "findOneAndDelete",
  async function (doc: ConversationInterface) {
    try {
      Message.deleteMany({ conversationId: doc._id });
      const proc = doc.users.map(async (user) => {
        const _user = await User.findById(user);
        if (_user) {
          const filteredConversations = _user.conversations.filter(
            (conversation: any) => !doc._id.equals(conversation)
          );
          const filteredConversationKeys = _user.conversationKeys.filter(
            (conversation) => !doc._id.equals(conversation.conversationId)
          );
          _user.set({
